...
B_PLUS_TREE_TYPE::BPlusTree(const std::string &name,
...
INDEX_TEMPLATE_ARGUMENTS
thread_local bool B_PLUS_TREE_TYPE::root_is_locked = false;

/*
 * Helper function to decide whether current b+tree is empty
 */
INDEX_TEMPLATE_ARGUMENTS
bool B_PLUS_TREE_TYPE::IsEmpty() const {
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
bool B_PLUS_TREE_TYPE::GetValue(const KeyType &key,
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
bool B_PLUS_TREE_TYPE::Insert(const KeyType &key, const ValueType &value,
                              Transaction *transaction) {
  {
    // only lock inside this scope
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
void B_PLUS_TREE_TYPE::StartNewTree(const KeyType &key,
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
bool B_PLUS_TREE_TYPE::InsertIntoLeaf(const KeyType &key,
                                      const ValueType &value,
                                      Transaction *transaction) {
  // find the leaf node
  ...
  if (leaf->GetSize() < leaf->GetMaxSize()) {
    ...
    // be equal.
    auto *leaf2 = Split<B_PLUS_TREE_LEAF_PAGE_TYPE>(leaf);
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
template <typename N> N *B_PLUS_TREE_TYPE::Split(N *node) {
  page_id_t page_id;
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
void B_PLUS_TREE_TYPE::InsertIntoParent(BPlusTreePage *old_node,
                                        const KeyType &key,
  ...
  if (old_node->IsRootPage()) {
    ...
    if (internal->GetSize() < internal->GetMaxSize()) {
      internal->InsertNodeAfter(old_node->GetPageId(), key,
                                new_node->GetPageId());
      // set ParentPageID
      ...
      page_id_t page_id;
      auto *page2 = buffer_pool_manager_->NewPage(page_id);
      if (page2 == nullptr) {
        throw Exception(EXCEPTION_TYPE_INDEX,
                        "all page are pinned while InsertIntoParent");
      }
      assert(page2->GetPinCount() == 1);
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
void B_PLUS_TREE_TYPE::Remove(const KeyType &key, Transaction *transaction) {
  if (IsEmpty()) {
    return;
  }
...
INDEX_TEMPLATE_ARGUMENTS
template <typename N>
bool B_PLUS_TREE_TYPE::CoalesceOrRedistribute(N *node,
                                              Transaction *transaction) {
  ...
  // 1. leaf node is a little bit different with internal node (key[0] is
  // reserved) when determine distribution or coalescing.
...
INDEX_TEMPLATE_ARGUMENTS
template <typename N>
void B_PLUS_TREE_TYPE::Coalesce(
...
INDEX_TEMPLATE_ARGUMENTS
template <typename N>
void B_PLUS_TREE_TYPE::Redistribute(N *neighbor_node, N *node, int index) {
  if (index == 0) {
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
bool B_PLUS_TREE_TYPE::AdjustRoot(BPlusTreePage *old_root_node) {
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
INDEXITERATOR_TYPE B_PLUS_TREE_TYPE::Begin() {
  KeyType key{};
...
INDEX_TEMPLATE_ARGUMENTS
INDEXITERATOR_TYPE
B_PLUS_TREE_TYPE::Begin(const KeyType &key) {
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
void B_PLUS_TREE_TYPE::UnlockUnpinPages(Operation op,
                                        Transaction *transaction) {
  if (transaction == nullptr) {
    return;
  }
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
template <typename N> bool B_PLUS_TREE_TYPE::isSafe(N *node, Operation op) {
...
B_PLUS_TREE_TYPE::FindLeafPage(const KeyType &key, bool leftMost, Operation op,
                               Transaction *transaction) {
  if (op != Operation::READONLY) {
    lockRoot();
    root_is_locked = true;
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
void B_PLUS_TREE_TYPE::UpdateRootPageId(bool insert_record) {
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
std::string B_PLUS_TREE_TYPE::ToString(bool verbose) {
  if (IsEmpty()) {
    return "Empty tree";
  }
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
void B_PLUS_TREE_TYPE::InsertFromFile(const std::string &file_name,
                                      Transaction *transaction) {
  int64_t key;
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
void B_PLUS_TREE_TYPE::RemoveFromFile(const std::string &file_name,
                                      Transaction *transaction) {
  int64_t key;
  std::ifstream input(file_name);
  while (input) {
