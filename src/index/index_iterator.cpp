...
IndexIterator<KeyType, ValueType, KeyComparator>::operator++()
{
  ...
  if (index_ == leaf_->GetSize() && leaf_->GetNextPageId() != INVALID_PAGE_ID)
  {
    ...
    if (page == nullptr) {
      throw Exception(EXCEPTION_TYPE_INDEX, "all pages are pinned while IndexIterator (operator++)");
    }
    ...
    auto next_leaf = reinterpret_cast<
        BPlusTreeLeafPage<KeyType, ValueType, KeyComparator> *>(page->GetData());
